...
list.innerHTML = `<div style="margin-bottom:12px"> <div style="display:flex;justify-content:space-between;font-size:11px;color:#555;margin-bottom:6px"> <span>今日の達成</span> <span style="color:${pct===100?'#10b981':'#818cf8'}">${doneCount}/${total} (${pct}%)</span> </div> <div style="height:6px;background:#1a1a2e;border-radius:99px;overflow:hidden"> <div style="height:100%;width:${pct}%;background:${pct===100?'#10b981':'#818cf8'};border-radius:99px;transition:width .5s"></div> </div> </div> ${habits.map(h => { const done = !!checks[h.id]; return`<div class="habit-item">
...
div.innerHTML = `<div style="font-size:10px;color:${color};letter-spacing:1px;text-transform:uppercase;margin-bottom:8px">📊 今日の総評${grade ? ' ' + grade : ''}</div> <div style="font-size:13px;color:#ccc;line-height:1.7">${comment}</div> ${allMealsDone ?`<div style="display:flex;gap:8px;margin-top:10px">
